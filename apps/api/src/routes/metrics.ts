  ...
  app.get('/metrics/products', {
    ...
    const products = await prisma.productPerformance.findMany({
      ...
      select: {
        ...
        unitsTrend: true,
        firstSeenAt: true,
        daysSinceFirstSale: true,
        revenueShare: true,
        topCrossSellProducts: true,
        collections: true,
        tags: true,
        historicalRoas: true,
        timesAdvertised: true,
        productTier: true,
      },
    });
    ...
    return {
      products: products.map((p) => ({
        ...
        revenuePrev30d: p.revenuePrev30d ? Number(p.revenuePrev30d) : null,
        revenueTrend: p.revenueTrend != null ? Number(p.revenueTrend) : null,
        unitsTrend: p.unitsTrend != null ? Number(p.unitsTrend) : null,
        firstSeenAt: p.firstSeenAt,
        daysSinceFirstSale: p.daysSinceFirstSale,
        revenueShare: p.revenueShare != null ? Number(p.revenueShare) : null,
        topCrossSellProducts: p.topCrossSellProducts as Array<{ title: string; coOccurrence: number }> | null,
        collections: p.collections as string[] | null,
        tags: p.tags as string[] | null,
        historicalRoas: p.historicalRoas != null ? Number(p.historicalRoas) : null,
        timesAdvertised: p.timesAdvertised,
